...
namespace StardewCapital.Core.Futures.Services
{
    ...
    public class NPCAgentManager
    {
            ...
            ScenarioParameters scenario)
        {
            ...
            // 8. 限制最大流量
            int clampedFlow = System.Math.Clamp((int)totalFlow, -_rules.VirtualFlow.MaxFlowPerTick, _rules.VirtualFlow.MaxFlowPerTick);
        ...
        private double CalculateTrendFlow(string symbol, double currentPrice, double strength)
        {
            ...
            // 价格在均线上方 -> 买入
            // 价格在均线下方 -> 卖出
            double signal = System.Math.Sign(currentPrice - ma);
        ...
        private double CalculateFOMOFlow(string symbol, double currentPrice, double strength, double asymmetricDown)
        {
            ...
            // 阈值检查
            if (System.Math.Abs(momentum) < _rules.NpcAgents.FomoTraders.Threshold)
                return 0;
